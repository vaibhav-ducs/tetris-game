...
class Grid {
  constructor(addScore) {
    this.cells = Array(C.BOARD_HEIGHT_CELLS)
      .fill(null)
      .map(() => Array(C.BOARD_WIDTH_CELLS).fill(null));
  ...
  blockCanMoveDown = (block) =>
    this.verifyBlockMoveValid(block, { rowChange: 1 });

  blockCanMoveLeft = (block) =>
    this.verifyBlockMoveValid(block, { colChange: -1 });

  blockCanMoveRight = (block) =>
    this.verifyBlockMoveValid(block, { colChange: 1 });

  blockCanRotate = (block) =>
    this.verifyBlockMoveValid(block, {}, block.getRotation());
  ...
  verifyBlockMoveValid = (block, change = {}, shape = block.getShape()) => {
    ...
    for (let i = 0; i < width; i++) {
      for (let j = 0; j < height; j++) {
        ...
        let targetCell =
          this.cells[block.getRow() + change.rowChange + j][
            block.getCol() + change.colChange + i
          ];

        if (blockCell && targetCell) {
          // Block collision
          return false;
        }
      }
  ...
  addBlockToCells = (block) => {
    ...
    const newCells = this.cells.map((row, y) =>
      y >= block.getRow() && y < block.getRow() + block.getHeight()
        ? [...row]
        : row,
    );
